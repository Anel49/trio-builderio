...
async function ensureIdentityVerificationColumn() {
  try {
    await pool.query(
      `alter table users add column if not exists pending_identity_ver boolean default null`,
    );
  } catch (e) {
...
export async function deletePhotoFromSession(req: Request, res: Response) {
  try {
    const { photoUrl, tempSessionId } = req.body || {};
    ...
    console.error("[deletePhotoFromSession] Error:", error);
    res.json({
      ok: true,
      message: "Photo removal processed",
      deleted: false,
