...
export default function OrderHistory() {
  ...
  useEffect(() => {
    if (ordersState.length > 0) {
      const ordersWithExt = ordersState.filter((o) => o.extension_of);
      console.log(
        `[OrderHistory] Total orders: ${ordersState.length}, Orders with extension_of: ${ordersWithExt.length}`,
      );
      if (ordersWithExt.length > 0) {
        console.log(
          "[OrderHistory] Orders with extension_of field:",
          ordersWithExt.map((o) => ({
            id: o.id,
            order_number: o.order_number,
            extension_of: o.extension_of,
          })),
        );
      }
    }
              ...
              sortedOrders.map((order) => (
                ...
                >
                  <CardContent className="p-0 flex flex-col">
                    {/* Header Section with ORDER PLACED, TOTAL, ORDER #, and STATUS */}
                    <div className="border-b px-6 py-4 grid grid-cols-1 md:grid-cols-[1fr_1fr_1fr_12rem] gap-6 bg-muted rounded-t-md">
                      {/* ORDER PLACED */}
                            ...
                            <div className="flex-1">
                              ...
                              <p className="text-sm text-muted-foreground mb-1">
                                {res.reservation_number
                                  ? `#${res.reservation_number}`
                                  : res.number
                                    ? `#${res.extension_of ? "EXT" : "REQ"}-${res.number}`
                              ...
                              {res.status.toLowerCase() === "confirmed" &&
                                reservationOrderNumbers.get(res.id) && (
                                  <button
                                    ...
                                    className="text-sm text-primary hover:underline cursor-pointer transition-colors mb-1 block"
                                  >
